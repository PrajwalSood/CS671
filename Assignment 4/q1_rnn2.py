...
model.add(tf.keras.layers.SimpleRNN(64, return_sequences=True, input_shape=(None, 2), activation = 'tanh'))
model.add(tf.keras.layers.SimpleRNN(64, return_sequences=False, activation = 'tanh'))
...
with open('metrics/RNN_hw2/acc.txt', 'w') as f:
  for i in acc:
    f.write(str(i) + '\n')

with open('metrics/RNN_hw2/loss.txt', 'w') as f:
  for i in loss:
    f.write(str(i.numpy()) + '\n')

with open('metrics/RNN_hw2/val_loss.txt', 'w') as f:
  for i in vloss:
    f.write(str(i.numpy()) + '\n')

model.save('models/RNN_hw2/model.h5')

acc = pd.read_csv('metrics/RNN_hw2/acc.txt', header=None).values
loss = pd.read_csv('metrics/RNN_hw2/loss.txt', header=None).values
val_loss = pd.read_csv('metrics/RNN_hw2/val_loss.txt', header=None).values
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.savefig('metrics/RNN_hw2/acc.png')
plt.show()
...
plt.savefig('metrics/RNN_hw2/loss.png')
plt.show()