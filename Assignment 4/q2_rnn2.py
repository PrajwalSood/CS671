...
with open('metrics/RNN_CV2/acc.txt', 'w') as f:
  for i in acc:
    f.write(str(i) + '\n')

with open('metrics/RNN_CV2/loss.txt', 'w') as f:
  for i in loss:
    f.write(str(i.numpy()) + '\n')

with open('metrics/RNN_CV2/val_loss.txt', 'w') as f:
  for i in vloss:
    f.write(str(i.numpy()) + '\n')

model.save('models/RNN_CV2/model.h5')

acc = pd.read_csv('metrics/RNN_CV2/acc.txt', header=None).values
loss = pd.read_csv('metrics/RNN_CV2/loss.txt', header=None).values
val_loss = pd.read_csv('metrics/RNN_CV2/val_loss.txt', header=None).values

loss_c = loss.copy()
loss = [np.average(loss[i : i+ 9]) for i in range(0,len(loss_c)-9,9)]

vloss = [np.average(val_loss[i : i+ 9]) for i in range(0,len(val_loss)-9,9)]
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.savefig('metrics/RNN_CV2/acc.png')
plt.show()
...
plt.savefig('metrics/RNN_CV2/loss.png')
plt.show()