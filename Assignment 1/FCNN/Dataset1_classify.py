...
X = pd.read_csv('data/Classification/LS_Group04/Class1.txt', sep=' ', header=None)
#read data from data/Classification/LS_Group04/Class*.txt
for i in range(2,4):
    data = pd.read_csv('data/Classification/LS_Group04/Class'+str(i)+'.txt', sep=' ', header=None)
    X = np.vstack((X, data.values))
...
print("Best Acc at {} hidden dims, acc:" .format(nets[best_i][1]), acc)
