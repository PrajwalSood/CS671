# -*- coding: utf-8 -*-
"""
Created on Sun Mar 20 16:42:09 2022
...
X = pd.read_csv('data/Regression/BivariateData/4.csv', header=None)
...
# network
    ...
    net.fit(X_train, y_train, epochs=10, learning_rate=5)
...
if best_i<4:
    ...
    best_i -=4
    net = Network()
    net.add(FCLayer(nets2[best_i][0], nets2[best_i][1]))
    net.add(ActivationLayer(sigmoid, sigmoid_prime))
    net.add(FCLayer(nets2[best_i][1], nets2[best_i][2]))
    net.add(ActivationLayer(sigmoid, sigmoid_prime))
    net.add(FCLayer(nets2[best_i][2], nets2[best_i][3]))
    net.add(ActivationLayer(sigmoid, sigmoid_prime))
    print("Best Error at {} hidden dims, acc:" .format(nets2[best_i][1:3]), acc)
# train
net.use(mse, mse_prime)
